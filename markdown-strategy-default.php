...
class markdown_strategy_default implements markdown_strategy
{
	...
	/**
	 * Main function. Performs some preprocessing on the input text and pass it through the document
	 * gamut.
	...
	/**
	 * Called after the transformation process to clear any variable which may be taking up memory
	 * unnecessarly.
	 * @return [type] [description]
	 */
	 ...
	 * stream. Passing $text to through this function gives a unique text-token which will be
	 * reverted back when calling unhash.
	 * @param  [type] $text     [description]
	 * @param  string $boundary Specify what character should be used to surround the token. By
	 *                          convention, "B" is used for block elements that needs not to be
	 *                          wrapped into paragraph tags at the end, ":" is used for elements
	...
	/**
	 * Run block gamut tranformations, without hashing HTML blocks. This is useful when HTML blocks
	 * are known to be already hashed, like in the first whole-document pass.
	 * @param  [type] $text [description]
	 * @return [type]       [description]
	 */
	...
	/**
	 * Process the contents of a single ordered or unordered list, splitting it into individual list
	 * items.
	 * @param  [type] $list_str      [description]
	 * @param  [type] $marker_any_re [description]
	 * @return [type]                [description]
	 */
	...
	private function doItalicsAndBold ( $text )
	{
		...
		while (1) {
			...
			// Each loop iteration search for the next emphasis token. Each token is then passed to handleSpanToken.
			...
			if ($tree_char_em) {
				...
				if ($token_len == 3) {
					...
					// Other closing marker: close one em or strong and change current token state to match the other
	...
	/**
	 * [formParagraphs description]
	 * @param  [type] $text string to process with html <p> tags
	 * @return [type]       [description]
	 */
	...
	{
	// Encode text for a double-quoted HTML attribute. This function is *not* suitable for
	...
	/**
	 * Smart processing for ampersands and angle brackets that need to be encoded. Valid character
	 * entities are left alone unless the no-entities mode is set.
	 * @param  [type] $text [description]
	 * @return [type]       [description]
	 */
	...
	/**
	 * Take the string $str and parse it into tokens, hashing embeded HTML, escaped characters and
	 * handling code spans.
	...
	/**
	 * Handle $token provided by parseSpan by determining its nature and returning the corresponding
	...
	{
		// For each line we separate the line in blocks delemited by tab characters. Then we
		// reconstruct every line by adding the appropriate number of space between each blocks.
	 ...
	 * (initially `mb_strlen`). If the function is not available, create a function that will
	 * loosely count the number of UTF-8 characters with a regular expression.
	 * @return [type] [description]
	 */
